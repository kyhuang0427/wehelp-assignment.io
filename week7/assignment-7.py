...
def get_member():
    ...
    if member:
        return jsonify({"data": member}), 200
    else:
...
def update_member_name():
        ...
        if not session.get('logged_in'):
            return jsonify({"error": True, "message": "會員未登入"}), 401
        ...
        if new_name:
            ...
            cursor.execute(update_query, (new_name, session['user_id']))
            connection.commit()
            return jsonify({"ok": True}), 200
        else:
            return jsonify({"error": True, "message": "姓名不能為空白"}), 400
